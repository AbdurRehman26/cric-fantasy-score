<div class="flex items-center" x-data="{ period: '{{ request()->query("period") ?? "1h" }}' }">
    <x-dropdown align="left" class="ml-2">
        ...
        <x-slot name="content">
            <x-dropdown-link :href="route('monitors.metrics', ['monitor' => $monitor, 'period' => '1h'])">
                1 Hour
            </x-dropdown-link>
            <x-dropdown-link :href="route('monitors.metrics', ['monitor' => $monitor, 'period' => '12h'])">
                12 Hours
            </x-dropdown-link>
            <x-dropdown-link :href="route('monitors.metrics', ['monitor' => $monitor, 'period' => '1d'])">
                1 Day
            </x-dropdown-link>
            <x-dropdown-link :href="route('monitors.metrics', ['monitor' => $monitor, 'period' => '7d'])">
                7 Days
            </x-dropdown-link>
            <x-dropdown-link :href="route('monitors.metrics', ['monitor' => $monitor, 'period' => '30d'])">
                30 Days
            </x-dropdown-link>
    ...
    <form
        ...
        action="{{ route("monitors.metrics", ["monitor" => $monitor, "period" => "custom"]) }}"
    >
